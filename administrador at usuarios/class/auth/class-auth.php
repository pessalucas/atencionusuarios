...
if ( ! class_exists( 'Auth' ) ) {
	...
	class Auth {
		...
		public function register_assets() {
			...
			wp_localize_script(
				...
				array(
					...
					'redirect' => get_permalink( $this->page_home_id ),
		...
		public function redirect_is_loggedin() {
			...
			// Si el usuario no esta logueado, lo mando al login page.
			...
			if ( is_user_logged_in() ){ 
				...
				if( ( in_array( "usuariosjefe", (array) $user->roles ) ) OR ( in_array( "usuarios", (array) $user->roles ) ) OR ( in_array( "control", (array) $user->roles ) ) ) { 
					// Si el usuario esta logueado y es la pagina de ingresar, lo redirigimos a la pagina de perfil.
					if ( is_page( $this->page_login_id ) ) {
						wp_redirect( get_permalink( $this->page_home_id ) );
						exit;
					}
				}else{
					wp_redirect( 'http://localhost/atencionusuarios/' );
						exit;
				}
			}
